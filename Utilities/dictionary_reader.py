		...
		for line in f:
			if "<entry id=" in line:
				...
				if "<orth" in line:
					...
					latin_dictionary[key] = prune_word(key, word)
				else:
					saved_key = key
				...
				word = line[word_start:word_end]
				latin_dictionary[saved_key] = prune_word(saved_key, word)

	f.close()
	return latin_dictionary


def prune_word(key, word):
...
def check_for_long_vowels(keyword, latin_dict):
	if keyword in latin_dictionary:
		# Return indices of long vowels
		word = latin_dict[keyword]
...
if __name__ == "__main__":
	latin_dictionary = parse_dictionary()
